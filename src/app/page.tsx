...
export default function Home() {
     ...
     */
    useEffect(() => {
        const socket = io("https://mediasoup-wfi6.onrender.com/mediasoup");
            ...
            ({
                ...
                transport?.on("produce", async (parameters: any, callback: any, errback: any) => {
                    ...
                    try {
                        ...
                        socket.emit("transport-produce", { kind, rtpParameters }, ({ id }: any) => {
    ...
    const createRecvTransport = async () => {
        ...
        socket.emit("createTransport", { sender: false }, ({ params }: { params: any }) => {
            if (params.error) {
                console.log(params.error);
                return;
            }
    ...
    const connectRecvTransport = async () => {
        ...
        await socket.emit("consumeMedia", { rtpCapabilities: device?.rtpCapabilities }, async ({ params }: any) => {
            if (params.error) {
                console.log(params.error);
                return;
            }
